...
class AppInitializationCubit extends Cubit<AppInitializationStatus> {
  ...
  Future<String> _resolveTargetedDownloadUrl(
      Map<String, dynamic> latestRelease, String fallbackUrl) async {
    final String? assetsUrl = latestRelease['assets_url'];
    if (assetsUrl == null || assetsUrl.isEmpty) return fallbackUrl;

    try {
      final response =
          await http.get(Uri.parse(assetsUrl), headers: _githubHeaders);
  ...
  Future<String> _generateCommitChangelog({
    ...
    try {
      final response =
          await http.get(Uri.parse(compareUrl), headers: _githubHeaders);
      if (response.statusCode != 200) {
      ...
      if (commits.isEmpty) {
        ...
        for (var entry in commits.reversed) {
          ...
          final String title =
              (commitMap['message'] ?? '').toString().split('\n').first.trim();
